...
| branch | `release-X.Y` branches exist across 8 repos | `make release VERSION=X` to advance to shipyard |
...
3. **Approve releases repo stage PRs or call `gh pr merge`** — auto-merge is set by the
   release scripts. You MAY (and should) approve dependency update PRs in downstream repos
   when their CI is not failing and no approval exists yet.
...
- **gh auth**: Must be logged in with `public_repo` scope for API queries and
  `pull-requests: write` permission for `gh pr review --approve` on downstream dep PRs.
...
**Important:** If `make release` fails and leaves the release YAML absent or incomplete,
the next poll may re-enter `not-started` and attempt to re-run `make release`. Report
the failure clearly and stop — a human must confirm the state is clean before retrying.

**Credential safety:** When reporting `make release` errors, redact any git remote URLs —
they may contain `GITHUB_TOKEN` embedded in `https://${GITHUB_TOKEN}:x-oauth-basic@github.com/...`.
Print only the error message text, never the full command.
...
Release v0.24.0-rc0 COMPLETE
  ...
  Images:   7/7 on quay.io
  Binaries: subctl published (linux/amd64, linux/arm64, darwin/amd64, darwin/arm64, windows/amd64)
  Krew:     post-release workflow triggered (verify succeeded — not a completion gate)
